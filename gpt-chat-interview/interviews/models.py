        ...
        if not self.pk and self.role != "assistant" and not self.chat.completed:
            ...
            if self.role == "user" and self.chat.messages.filter(role="assistant").count() >= 5:
                Message.objects.create(
                    chat=self.chat,
                    role="system",
                    content="Realize o feedback do candidato, esse feedback deve informar quais foram os pontos positivos, os pontos negativos, o que pode ser melhorado e se o candidato possui um perfil adequado para a vaga."
                )
